...
# %% auto 0
__all__ = ['DocLinks', 'get_patch_name', 'build_modidx', 'nbglob', 'convert_to_black', 'nbprocess_export']
...
def convert_to_black():
    "Format exported modules to black standard"
    dest = config_key('lib_path')
    if os.environ.get('IN_TEST',0): return
    for file in dest.glob("**/*.py"):
        if file.name[0]!='_': 
            ...
            with open(file,'w',encoding='utf8') as f:
                f.write(code)
    modidx = dest/'_modidx.py'
    with open(modidx) as f:
        code = f.read()
    code = black.format_str(code, mode=black.Mode())
    with open(modidx,'w',encoding='utf8') as f:
        f.write(code)
...
def nbprocess_export(
    ...
    build_modidx()
    black_compat = config_key('black_compat', default='False',path=False).lower() == 'true'
    if black_compat:
        convert_to_black()

...
# %% auto 0
__all__ = ['find_var', 'read_var', 'update_var', 'ModuleMaker', 'make_code_cell', 'make_code_cells', 'decor_id', 'retr_exports',
           'LibraryMaker', 'relative_import', 'update_import', 'basic_export_nb2']
...
class ModuleMaker:
    "Helper class to interact with python files from notebook source cells"
    ...
    def write_contents(self, cells=None, mode="w", autogenerate=False, offset=0):
        "Writes to `self.fname` in `mode` with `starting_string` followed by `cells`."
        with self.fname.open(mode) as f:
            if autogenerate: f.write(f"# AUTOGENERATED! DO NOT EDIT! File to edit: {self.dest2nb}.")
            if cells is not None: 
                if isinstance(cells, str): f.write(cells)
                else: write_cells(cells, self.hdr, f, offset)
            f.write("\n")
            
    def _make_exists(self, cells):
        "`make` for `is_new=False`"
        self.write_contents(cells, "a")
    
    def make(self, cells):
        "Creates and writes modules from `cells`"
        if not self.is_new: return self._make_exists(cells)
        self.fname.parent.mkdir(exist_ok=True, parents=True)
        self.write_contents(cells, autogenerate=True)

# %% ../nbs/02_maker.ipynb 17
def make_code_cell(code:str, idx_:int=None): 
    "Creates a simple `code` cell with the contents of `code`"
    return AttrDict(source=code, cell_type="code", execution_count=None, idx_=idx_)

def make_code_cells(*ss:str): 
    "Creates multiple `code` cells based on `ss`"
    return dict2nb({'cells':L(ss).map(make_code_cell)}).cells
...
def make(self:LibraryMaker, cells, all_cells=None, lib_name=None):
    ...
    if not self.is_new: 
        ...
        return super(LibraryMaker,self).make(cells)
...
def basic_export_nb2(fname, name, dest=None):
    ...
    LibraryMaker(dest=dest, name=name, nb_path=fname).make(cells)
